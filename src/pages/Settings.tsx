    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 28 }}>
        <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* Plan */}
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border1)', borderRadius: 16, padding: 24 }}>
            <h2 style={{ fontSize: 14, fontWeight: 600, margin: '0 0 4px' }}>Current Plan</h2>
            <p style={{ fontSize: 12, color: 'var(--text3)', margin: '0 0 16px' }}>Manage your subscription. Upgrades take effect immediately.</p>
              ...
              {PLANS.map(plan => {
                ...
                return (
                  <div key={plan.id} style={{ border: `1px solid ${isCurrent ? 'var(--green)' : 'var(--border1)'}`, borderRadius: 12, padding: 16, background: isCurrent ? 'rgba(34,214,135,0.05)' : 'var(--bg3)' }}>
                    ...
                    <div style={{ fontFamily: 'DM Serif Display, serif', fontSize: 22, marginBottom: 12 }}>${plan.price}<span style={{ fontSize: 12, fontFamily: 'Syne, sans-serif', color: 'var(--text3)' }}>/mo</span></div>
                    {isCurrent
                      ? <div style={{ fontSize: 11, color: 'var(--green)', fontFamily: 'DM Mono, monospace' }}>✓ CURRENT PLAN</div>
                      : <button style={{ width: '100%', padding: '7px', background: 'var(--bg4)', border: '1px solid var(--border2)', borderRadius: 7, color: 'var(--text2)', fontFamily: 'Syne, sans-serif', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                          onClick={() => toast.info('Stripe billing portal — integrate with your Stripe webhook')}>
                          Upgrade
                        </button>
                    }
                  </div>
