...
export default function AccountsPayable() {
  ...
  // Logic: Filter outstanding bill entries (amount < 0 and not fully paid)
  const outstandingEntries = entries.filter(e => e.amount < 0 && e.payment_status !== 'paid')

  // Group by contact_id
  const apByContact = outstandingEntries.reduce((acc, e) => {
    const key = e.contact_id || 'Unknown'
    const balance = Math.abs(e.amount + (e.paid_amount || 0)) // balance = |amount| - paid_amount (since amount is negative)
    // Actually simpler: if amount is -100 and paid is 20, balance is |-100 + 20| = 80. Correct.
    if (!acc[key]) {
      acc[key] = {
        contact_id: key,
        contact_name: e.contact_name || 'Generic Vendor',
        amount: 0
      }
    }
  ...
  const apRows = Object.values(apByContact).sort((a, b) => b.amount - a.amount)
  
  const totalAP = apRows.reduce((sum, row) => sum + row.amount, 0)
  const vendorCount = apRows.length
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ height: 56, background: 'var(--bg2)', borderBottom: '1px solid var(--border1)', display: 'flex', alignItems: 'center', padding: '0 28px' }}>
        <h1 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>Accounts Payable</h1>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 28 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 24 }}>
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border1)', borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono, monospace', letterSpacing: '0.5px', marginBottom: 10 }}>TOTAL A/P</div>
            <div style={{ fontSize: 26, fontWeight: 700, letterSpacing: -1, fontFamily: 'DM Serif Display, serif', color: 'var(--red)' }}>
              {fmt(totalAP)}
            </div>
          </div>
          ...
          {/* KPI Card: # Vendors */}
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border1)', borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 10, color: 'var(--text3)', fontFamily: 'DM Mono, monospace', letterSpacing: '0.5px', marginBottom: 10 }}>OUTSTANDING VENDORS</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: 'var(--bg3)' }}>
                <th style={{ textAlign: 'left', padding: '11px 16px', fontSize: 10, fontFamily: 'DM Mono, monospace', color: 'var(--text3)', letterSpacing: '0.5px', borderBottom: '1px solid var(--border1)', fontWeight: 500 }}>VENDOR NAME</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={3} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>No outstanding vendor balances</td></tr>
              ) : apRows.map(row => (
                <tr key={row.contact_id} style={{ transition: 'background 0.1s' }}
                  ...
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontFamily: 'DM Mono, monospace', borderBottom: '1px solid var(--border1)', color: 'var(--red)', fontWeight: 600 }}>{fmt(row.amount)}</td>
